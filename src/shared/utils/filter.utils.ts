...
import { FieldType, Operation } from '../dtos/paginate.req.dto';
...
export const buildFilter = (query, search: object | object[] = null) => {
      ...
      case FieldType.DATE:
        ...
        switch ((filter?.operation ?? '').toUpperCase()) {
          ...
          case Operation.BETWEEN:
            value = Between(new Date(filter.value[0]), new Date(filter.value[1]));
            break;
          case Operation.NOT_BETWEEN:
            value = Not(Between(new Date(filter.value[0]), new Date(filter.value[1])));
            break;
          case Operation.GREATER_THAN:
  ...
  searchArray.forEach((item) => deepMerge(andWhere, item));
